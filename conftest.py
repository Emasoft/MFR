...
def temp_test_dir(tmp_path: Path):
    ...
    (runtime_dir / "atlasvibe_root" / "sub_atlasvibe_folder" / "another_ATLASVIBE_dir").mkdir()
    deep_file = runtime_dir / "atlasvibe_root" / "sub_atlasvibe_folder" / "another_ATLASVIBE_dir" / "deep_atlasvibe_file.txt"
    deep_file.write_text("This file contains ATLASVIBE multiple times: Atlasvibe atlasVibe")
    
    # Create excluded items in runtime directory
    (runtime_dir / "excluded_atlasvibe_dir").mkdir()
    (runtime_dir / "excluded_atlasvibe_dir" / "excluded_file.txt").write_text("ATLASVIBE content")
    (runtime_dir / "exclude_this_atlasvibe_file.txt").write_text("Atlasvibe exclusion test")
    
    # Verify structure
    assert (runtime_dir / "atlasvibe_root").exists(), "Required dir not created in fixture"
    context = {
        "runtime": runtime_dir,
        "config": config_dir
    }
