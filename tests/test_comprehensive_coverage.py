...
class TestScanAndExecute:
    ...
    def test_scan_with_large_file(self, temp_dir, large_file, mapping_file):
        ...
        original_threshold = file_system_operations.LARGE_FILE_SIZE_THRESHOLD
        file_system_operations.LARGE_FILE_SIZE_THRESHOLD = 1000  # 1KB
        ...
        try:
            ...
            file_system_operations.LARGE_FILE_SIZE_THRESHOLD = original_threshold
        ...
        transactions = [
            {
                "id": "1",
                "line_number": 1,
                "original_line": "OldName line 1",
                "new_line": "NewName line 1"
            },
            {
                "id": "2",
                "line_number": 3,
                "original_line": "OldName line 3",
                "new_line": "NewName line 3"
            }
        ]
        
        # Normal execution
        results = _execute_file_content_batch(
            str(test_file),
            transactions,
            "utf-8",
            dry_run=False
...
class TestReplaceLogic:
    ...
    def test_load_replacement_map_all_errors(self, tmp_path):
        ...
        empty_key.write_text(json.dumps({
            "REPLACEMENT_MAPPING": {
        ...
        empty_value.write_text(json.dumps({
            "REPLACEMENT_MAPPING": {
                "key": "",
                "valid": "replacement"
            }
        ...
        no_valid.write_text(json.dumps({
            "REPLACEMENT_MAPPING": {
                "": "empty_key",
                "empty_value": ""
            }
        ...
        recursive.write_text(json.dumps({
            "REPLACEMENT_MAPPING": {
                "a": "b",
                "b": "c",
        ...
        with patch('re.compile', side_effect=re.error("Invalid regex")):
            ...
            good_map.write_text(json.dumps({
                "REPLACEMENT_MAPPING": {"test": "replacement"}
            }))
...
class TestCLIAndMain:
    ...
    def test_check_existing_transactions_comprehensive(self, tmp_path, mock_logger):
        ...
        all_done.write_text(json.dumps([
            {"status": TransactionStatus.COMPLETED.value},
            {"status": TransactionStatus.COMPLETED.value}
        ]))
