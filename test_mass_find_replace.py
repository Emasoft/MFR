...
def test_dry_run_behavior(temp_test_dir: dict, default_map_file: Path, assert_file_content):
    ...
    orig_deep_file_path = context_dir / "atlasvibe_root" / "sub_atlasvibe_folder" / "another_ATLASVIBE_dir" / "deep_atlasvibe_file.txt"
    original_content = orig_deep_file_path.read_text(encoding='utf-8')

    assert original_content == "This file contains ATLASVIBE multiple times: Atlasvibe atlasVibe"
...
def test_folder_nesting(temp_test_dir: dict, default_map_file: Path):
    ...
    assert test_folders == ["atlasvibe_a", "atlasvibe_a/atlasvibe_b"], "Folders not processed from shallow to deep"
...
def test_binary_files_logging(temp_test_dir, default_map_file):
    ...
    bin_content = b'Header' + b'ATLASVIBE' + b'\x00\x01' + b'atlasVibe' + b'\x02' + b'Atlasvibe' + b'Footer'
